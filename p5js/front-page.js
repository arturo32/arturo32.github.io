...
class Arrow {
	...
	update() {
		...
		if(isOverButton) {
			...
			if(imaginaryRadius > abs(xDiff) + abs(yDiff)) {
				fill(lerpColor(color(this.color), color(currentColor), map(imaginaryRadius - (abs(xDiff) + abs(yDiff)), 0, 800, 0, 1)));
			}

			this.rotation = TAU/2 - atan2(yDiff, xDiff);
		}
		else {
	...
	show() {
		...
		beginShape();
			vertex(sin(p1)*this.scale, cos(p1)*this.scale);
			vertex(sin(p2)*this.scale, cos(p2)*this.scale);
			vertex(sin(p3)*this.scale*0.5, cos(p3)*this.scale*0.5);
			vertex(sin(p4)*this.scale, cos(p4)*this.scale);
		endShape(CLOSE);
...
function setButtonFunctions(buttonId, color) {
	let buttonElement = select(buttonId);
...
function isForbiddenArea(x, y) {
	...
	let titleElement = select('header h1').elt;
	...
	let aboutButtonElement = select('#aboutHomeButton').elt;
	...
	let lastButtonElement = select('#othersHomeButton').elt;
...
function setup() {
	...
	let frontPageCanvas = select('#frontPageCanvas');
	...
	for(let i = 0; i < 15; ++i) {
		let x = random(canvasWidth/20, canvasWidth - canvasWidth/20);
		let y = random(canvasHeight/10, canvasHeight - canvasHeight/10);
		
		while(isForbiddenArea(x, y)) {
			x = random(canvasWidth/20, canvasWidth - canvasWidth/20);
			y = random(canvasHeight/10, canvasHeight - canvasHeight/10);
		}
